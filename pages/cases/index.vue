	...
	<div class="grid gap-10">
		<div class="flex flex-col gap-10">
			...
			<h1 class="text-lg sm:text-xl md:text-3xl lg:text-4xl font-medium text-center">
				{{ translations['cases.title'] }}</h1>
		</div>
		<div class="sm:container sm:max-w-[1760px]">
			<div class="sm:bg-background rounded-[40px] py-20">
				<div class="container grid grid-cols-1 lg:grid-cols-2 gap-6">
					<div class="sm:inline-flex hidden items-center flex-col sm:flex-row gap-4 sm:gap-6 border rounded-2xl border-grey-1 group p-2 sm:p-[8px_24px_8px_8px]"
						v-for="item in cases.results" :key="item.id">
						<div class="max-w-56 max-h-56 flex-shrink-0 overflow-hidden rounded-lg">
							<img :src="item.image" alt=""
								class="w-full h-full object-cover sm:rounded-[18px_8px_8px_18px]" />
						</div>
							...
							<a :href="item.url" target="_blank">
								<Button variant="secondary" class="bg-grey-0">
									...
									<svg xmlns="http://www.w3.org/2000/svg" width="20" height="21" viewBox="0 0 20 21"
										fill="none">
										<path d="M5.8335 14.6665L9.16683 10.4998L5.8335 6.33317" stroke="#272727"
											stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" />
										<path d="M10.8335 14.6665L14.1668 10.4998L10.8335 6.33317" stroke="#272727"
											stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" />
									</svg>
								</Button>
							</a>
					...
					<div class="flex sm:hidden border flex-col gap-4 p-6 rounded-[16px]" v-for="item in cases.results"
						:key="item.id">
