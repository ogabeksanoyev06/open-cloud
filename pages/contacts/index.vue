<template>
	<div class="grid gap-10 pt-4">
		...
		<div class="sm:container sm:max-w-[1760px]">
			<div class="bg-background py-6 sm:py-20 sm:rounded-[40px]">
				<div class="container flex flex-col gap-10 sm:gap-20">
					<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
						<div class="flex flex-col gap-10 bg-grey-0 rounded-2xl p-6">
							...
							<div class="flex gap-4">
								<a v-for="(number, idx) in contacts.nbm.split(',')" target="_blank" :key="idx"
									class="font-medium text-xl" :href="`tel:${number}`">{{ number }}</a>
							</div>
						</div>
						<div class="flex flex-col gap-10 bg-grey-0 rounded-2xl p-6">
							<p class="text-grey">{{ translations['contacts.email'] }}</p>
							<a :href="`mailto:${contacts.email}`" target="_blank" class="text-xl font-medium">{{
								contacts.email }}</a>
						</div>
						<div class="flex flex-col gap-10 bg-grey-0 rounded-2xl p-6">
								...
								<a :href="contacts.telegram" target="_blank">
									<svg xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 32 32"
										...
										<path
											...
											fill="white" />
										<defs>
											<linearGradient id="paint0_linear_528_11758" x1="16" y1="2" x2="16" y2="30"
								...
								<a :href="contacts.facebook" target="_blank">
									<svg xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 32 32"
										...
										<path
											...
											fill="white" />
										<defs>
											<linearGradient id="paint0_linear_528_11759" x1="16" y1="2" x2="16"
					...
					<div class="grid lg:grid-cols-[400px_minmax(0,1fr)] gap-10 lg:items-start">
						<div class="flex flex-col gap-6">
							<h3 class="text-xl sm:text-2xl">{{ translations['contacts.form-title'] }}</h3>
								...
								<div class="grid gap-4">
									<div class="grid gap-2">
										<VField name="name" rules="required" v-model="form.name">
											...
											<Input v-model="form.name" id="login" type="text"
												:placeholder="translations['contacts.name-input']" />
									...
									<div class="grid gap-2">
										<VField name="email" rules="required|email" v-model="form.email">
											...
											<Input v-model="form.email" id="email" type="text"
												:placeholder="translations['contacts.email-input']" />
									...
									<div class="grid gap-2">
										<VField name="message" rules="required" v-model="form.message">
											...
											<Textarea v-model="form.message" id="message" type="text"
												:placeholder="translations['contacts.message-input']" />
											<span class="text-sm text-destructive font-medium">{{ errors.message
												}}</span>
										</VField>
									</div>
									...
									<Button type="submit" :disabled="loading">
										...
										<svg xmlns="http://www.w3.org/2000/svg" width="20" height="20"
											...
											<path d="M5.8335 14.168L9.16683 10.0013L5.8335 5.83464" stroke="#272727"
												stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" />
											<path d="M10.8335 14.168L14.1668 10.0013L10.8335 5.83464" stroke="#272727"
												stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" />
										</svg>
									</Button>
								</div>
						...
						<div v-html="contacts.map"
							class="min-h-[360px] lg:min-h-[500px] h-full rounded-3xl overflow-hidden"></div>
					</div>
