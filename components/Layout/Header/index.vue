<template>
	<transition name="fade">
		<div v-if="isMenuOpen" @click="isMenuOpen = false"
			class="hidden lg:block w-screen h-screen fixed top-0 left-0 z-[8] bg-black/[0.4] backdrop-blur-sm"></div>
	</transition>
	<header id="header" class="sticky z-40 top-0 bg-background backdrop-blur-lg">
		<div class="container flex h-16 sm:h-[100px] items-center">
			<div class="mr-4 lg:mr-1 flex items-center">
				...
				<nav
					class="hidden lg:flex flex-wrap items-center max-lg:space-x-4 lg:space-x-8 xl:space-x-10 text-base font-normal">
					<Button variant="ghost" class="p-0 gap-2 hover:bg-transparent text-base font-normal"
						...
						<span>
							<svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 20 20"
								fill="none">
								<path d="M5.8335 8.33325L10.0002 11.6666L14.1668 8.33325" stroke="#272727"
									stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" />
							</svg>
						</span>
					</Button>
					<NuxtLink :to="localePath('/about-us')" class="transition-300">{{ translations['header.link2'] }}
					</NuxtLink>
					<NuxtLink :to="localePath('/calculator')" class="transition-300">{{ translations['header.link3'] }}
					</NuxtLink>
					<NuxtLink :to="localePath('/documentation')" class="transition-300">{{ translations['header.link4']
						}}
					</NuxtLink>
					<NuxtLink :to="localePath('/partners')" class="transition-300">{{ translations['header.link5'] }}
					</NuxtLink>
					<NuxtLink :to="localePath('/contacts')" class="transition-300">{{ translations['header.link6'] }}
					</NuxtLink>
				</nav>
			</div>
			<button @click="isOpen = true" class="lg:hidden flex justify-end flex-1">
				<svg xmlns="http://www.w3.org/2000/svg" width="40" height="40" viewBox="0 0 40 40" fill="none">
					<path d="M11.6665 13.3333H28.3332M11.6665 19.9999H28.3332M11.6665 26.6666H28.3332" stroke="#272727"
			...
			<div class="hidden lg:flex flex-1 items-center space-x-2 justify-end">
				<DropdownMenu>
					<DropdownMenuTrigger as-child>
						<Button variant="secondary" class="">
							...
							<span>
								<svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 20 20"
									fill="none">
									<path d="M5.83325 8.33325L9.99992 11.6666L14.1666 8.33325" stroke="#272727"
										stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" />
								</svg>
							</span>
						</Button>
					...
					<DropdownMenuContent align="end" class="flex flex-col gap-1">
						<DropdownMenuItem @click="selectLang(lang)" v-for="(lang, i) in langs" :key="i"
							class="cursor-pointer p-1 hover:bg-secondary rounded">
							{{ lang.label }}
						</DropdownMenuItem>
					</DropdownMenuContent>
				</DropdownMenu>
				...
				<Button class="gap-2">
					...
					<svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 20 20" fill="none">
						<ellipse cx="10.0001" cy="14.5834" rx="5.83333" ry="2.91667" stroke="#272727" stroke-width="1.5"
							stroke-linejoin="round" />
						<ellipse cx="10.0001" cy="5.83333" rx="3.33333" ry="3.33333" stroke="#272727" stroke-width="1.5"
							stroke-linejoin="round" />
					</svg>
				</Button>
			</div>
		...
		<transition name="modal">
			<aside v-if="isMenuOpen"
				class="hidden lg:block absolute p-6 transition-300 w-full z-[9] left-0 top-16 sm:top-[100px] bg-grey-0">
				<div class="container">
					<div class="grid lg:grid-cols-[400px_minmax(0,1fr)] gap-6 items-start">
						<ul class="flex flex-col gap-6">
							<li v-for="category in categories.results" @click="linkTrigger(category.slug)"
								:key="category.id"
						...
						<transition name="modal-icon">
							<div class="flex flex-col gap-6 bg-white p-10 rounded-2xl">
								...
								<nav class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
									<NuxtLink :to="localePath(`/products/${item.slug}`)" class="flex flex-col gap-2"
										v-for="item in categoriesInner.products" :key="item.id">
					...
					<nav class="flex flex-col gap-8 mt-6">
						<button @click="activeSheet" class="flex items-center justify-between font-medium w-full">
							...
							<svg xmlns="http://www.w3.org/2000/svg" width="20" height="21" viewBox="0 0 20 21"
								fill="none">
								<path d="M11.6667 7.16675L15 10.5001M15 10.5001L11.6667 13.8334M15 10.5001L5 10.5001"
									stroke="#272727" stroke-width="1.5" stroke-linecap="round"
									stroke-linejoin="round" />
							</svg>
						</button>
						<NuxtLink :to="localePath('/calculator')">{{ translations['header.link2'] }}</NuxtLink>
						<NuxtLink :to="localePath('/about-us')">{{ translations['header.link3'] }}</NuxtLink>
					...
					<div class="flex flex-col gap-6">
						<DropdownMenu>
							<DropdownMenuTrigger as-child class="w-full justify-between">
								<Button variant="secondary" class="!bg-transparent p-0">
									Ру
									<span>
										<svg xmlns="http://www.w3.org/2000/svg" width="20" height="20"
											viewBox="0 0 20 20" fill="none">
						...
						<DropdownMenu>
							...
							<DropdownMenuContent align="start" class="flex flex-col gap-2 p-3 w-full">
								<a v-for="contact in formattedContacts" :key="contact" :href="`tel:${contact}`"
									target="_blank">
									{{ contact }}
								</a>
							</DropdownMenuContent>
						...
						<Button class="w-full">
							...
							<svg xmlns="http://www.w3.org/2000/svg" width="20" height="21" viewBox="0 0 20 21"
								fill="none">
								<ellipse cx="9.99984" cy="15.0833" rx="5.83333" ry="2.91667" stroke="#272727"
									stroke-width="1.5" stroke-linejoin="round" />
								<ellipse cx="9.99984" cy="6.33333" rx="3.33333" ry="3.33333" stroke="#272727"
									stroke-width="1.5" stroke-linejoin="round" />
							</svg>
						</Button>
					</div>
				...
				<div class="flex flex-col h-full gap-6">
					<h3 class="font-medium text-lg flex gap-2 items-center" @click="closeProductModal">
						<svg xmlns="http://www.w3.org/2000/svg" width="20" height="21" viewBox="0 0 20 21" fill="none">
							<path d="M11.6665 6.33325L8.33317 10.4999L11.6665 14.6666" stroke="#272727"
								stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" />
						</svg>
						{{ translations['header.link1'] }}
					</h3>
					<Accordion type="single" class="w-full" collapsible>
						<AccordionItem class="border-none" v-for="category in categories.results" :key="category.id"
							:value="category.slug">
							<AccordionTrigger class="text-sm font-medium to-grey hover:no-underline"> {{ category.title
								}} </AccordionTrigger>
							<AccordionContent>
								<nav class="flex flex-col gap-4">
									<NuxtLink v-for="item in categoriesInner.products"
										:to="localePath(`/products/${item.slug}`)" :key="item.id"
